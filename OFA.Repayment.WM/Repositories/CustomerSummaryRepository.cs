...
{
    public class CustomerSummaryRepository : ICustomerSummaryRepository
    {
        private readonly IOFAEventStore _eventStore;
        public CustomerSummaryRepository(IOFAEventStore eventStore)
        {
            _eventStore = eventStore;
        }

        public async Task<IEnumerable<IEvent>> GetAllAsync(string id)
            => await _eventStore.ReadAllEventsASync<CustomerSummaryCreated>(id);

        public async Task SaveAsync(string streamName, IEvent @event)
            => await _eventStore.AppendEventAsync("customer-summary", @event);
    }
}
