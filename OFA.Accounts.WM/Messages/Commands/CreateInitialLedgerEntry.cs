...
namespace OFA.Accounts.WM.Messages.Commands
{
    public class CreateInitialLedgerEntry : ICommand
    {
        ...
        public CreateInitialLedgerEntry(int custId, int debit, int credit, int balance, string details, Guid correlationId, int? seasonId = null)
        {
        ...
        private IEvent Apply()
        {
            try
            {
                return new InitialLedgerEntryCreated(CustomerId, (int)SeasonId, AccountName, Debit, Credit, Balance, Details, CorrelationId);
            }
            catch (Exception)
            {
                return null;
